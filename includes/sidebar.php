
<section class="sidebarSec">
    <div class="side-main">
        <div class="logo-side">
            <a href="index.php">
                <img src="images/logo.png" alt="img" class="img-fluid">
            </a>
            <button class="btn toggler">
                <i class="fa-solid fa-bars"></i>
            </button>
        </div>
        <div class="side-box">
            <ul class="side-box-ul">
                <h4>Menu</h4>
                <li class="side-box-li"><a href="index.php" class="active"><i class="fa-solid fa-gauge-high"></i>
                        <span>Dashsboards</span></a>
                </li>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-solid fa-rocket"></i> <span>Apps</span></a>
                </li>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-solid fa-layer-group"></i>
                        <span>Layouts</span></a></li>
                <h4>Pages</h4>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-solid fa-user"></i>
                        <span>Authentication</span></a>
                </li>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-regular fa-file-lines"></i>
                        <span>Pages</span></a>
                </li>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-solid fa-rocket"></i>
                        <span>Landing</span></a></li>
                <h4>Components</h4>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-solid fa-pen-ruler"></i> <span>Base
                            UI</span></a></li>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-brands fa-canadian-maple-leaf"></i>
                        <span>Advance
                            UI</span></a>
                </li>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-solid fa-bridge"></i>
                        <span>Widgets</span></a></li>
                <li class="side-box-li"><a href="form.php"><i class="fa-solid fa-file-waveform"></i>
                        <span>Forms</span></a></li>
                <li class="side-box-li"><a href="table.php"><i class="fa-solid fa-table"></i>
                        <span>Tables</span></a></li>
                <li class="side-box-li"><a href="product.php"><i class="fa-solid fa-chart-gantt"></i>
                        <span>Charts</span></a></li>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-solid fa-icons"></i>
                        <span>Icons</span></a></li>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-solid fa-location-dot"></i>
                        <span>Maps</span></a></li>
                <li class="side-box-li"><a href="javascript:;"><i class="fa-brands fa-docker"></i> <span>Multi
                            Level</span></a>
                </li>
            </ul>
        </div>
    </div>
</section>